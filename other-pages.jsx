...
function EventsTab({ lang }) {
  const evList = (window.events || []).slice().sort((a, b) => (a.date || '').localeCompare(b.date || ''));
...
function JoinPage({ navigate, projectId }) {
  ...
  const roleDescCard = (project && savedRole) ? (() => {
    const desc = typeof getRoleDescription === 'function' ? getRoleDescription(savedRole, lang) : null;
    if (!desc) return null;
    return (
      <div style={{
        padding: '18px 20px',
...
Object.assign(window, { BlogPage, JoinPage, EventsTab });
