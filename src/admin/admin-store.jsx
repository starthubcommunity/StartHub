...
function mapPostToDb(item) {
  return {
    ...
    bg:          item.bg          || 'var(--blue-light)',
    image_url:   item.cover       || null,
    image_alt:   item.imageAlt    || null,
    aspect_ratio: item.aspectRatio ?? 1.78,
    guest_author: item.guestAuthor || null,
    linkedin_share:  item.linkedinShare  ?? false,
    linkedin_posted: item.linkedinPosted ?? false,
...
function mapPostFromDb(row) {
  return {
    ...
    linkedinPosted: row.linkedin_posted ?? false,
