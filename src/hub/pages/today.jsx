        ...
        <>
          ...
          <Block title="Süresi gelen takipler">
            {dueFollowUps.map(({ t, c }) => (
              <Row key={t.id} onClick={() => setOpenId(c.id)} main={c.fullName} meta={`takip: ${fmt(t.followUpAt)}`} />
            ))}
          </Block>
          ...
          <Block title="Süresi dolan kapılar">
            {dueGates.map(({ g, c }) => (
              <Row key={g.id} onClick={() => setOpenId(c.id)} main={c.fullName} meta={`Kapı ${g.gate} · vade ${fmt(g.dueAt)}`} />
            ))}
          </Block>
